...
<tr>
	<td width="100" >{#winPeriode_projet#} :</td>
	<td>
		<select name="projet_id" id="projet_id" class="span3">
			<option value="">- - - - - - - - - - - - - - - -</option>
			{foreach from=$listeProjets item=projet}
				<option value="{$projet.projet_id}" {if $periode.projet_id eq $projet.projet_id}selected="selected">{$projet.nom} ({$projet.projet_id}) {if $projet.livraison neq ''} - S{$projet.livraison}</option>
			{/foreach}
		</select>
		&nbsp;&nbsp;&nbsp;&nbsp;
		{#winPeriode_filtrage#} :
...
<tr>
	<td >{#winPeriode_user#} :</td>
	<td>
		<select name="user_id" id="user_id">
		{foreach from=$listeUsers item=userTmp}
			<option value="{$userTmp.user_id}" {if $periode.user_id eq $userTmp.user_id}selected="selected" {if isset($user_id_choisi) && $user_id_choisi eq $userTmp.user_id}selected="selected">{$userTmp.nom} - {$userTmp.user_id}</option>
		{/foreach}
		</select>
...
<tr id="bloc_date_fin">
	<td  valign="middle">{#winPeriode_fin#} :</td>
	<td>
...
<tr>
	<td >{#winPeriode_statut#} :</td>
	<td>
		<select name="statut_tache" id="statut_tache">
			<option value="a_faire" {if $periode.statut_tache eq "a_faire"}selected="selected">{#winPeriode_statut_a_faire#}</option>
			<option value="en_cours" {if $periode.statut_tache eq "en_cours"}selected="selected">{#winPeriode_statut_en_cours#}</option>
			<option value="fait" {if $periode.statut_tache eq "fait"}selected="selected">{#winPeriode_statut_fait#}</option>
			<option value="abandon" {if $periode.statut_tache eq "abandon"}selected="selected">{#winPeriode_statut_abandon#}</option>
		</select>
	</td>
</tr>
<tr>
	<td >{#winPeriode_livrable#} :</td>
	<td>
		<select name="livrable" id="livrable">
			<option value="oui" {if $periode.livrable eq "oui"}selected="selected">{#oui#}</option>
			<option value="non" {if $periode.livrable eq "non"}selected="selected">{#non#}</option>
		</select>
	</td>
</tr>
<tr>
	<td  nowrap="nowrap">{#winPeriode_titre#} :</td>
	<td>
		<div class="ui-widget">
...
<tr>
	<td  nowrap="nowrap">{#winPeriode_commentaires#} :</td>
	<td>
		<textarea style="width:360px;height:36px;" id="notes" name="notes" class="textareaAuto">{$periode.notes_xajax|escape:"html"}</textarea>
	</td>
</tr>
<tr>
	<td  nowrap="nowrap">{#winPeriode_lien#} :</td>
	<td>
		<div class="input-append">
			<input type="text" name="lien" id="lien" size="40" maxlength="2000" value="{$periode.lien}" style="width:318px" />
			{if $periode.lien neq ""}
				<a class="btn btn-small" type="submit" style="padding-top:4px;" id="btnGotoLien" rel="tooltip" title="{#winPeriode_gotoLien#|escape}" href="{if $periode.lien|strpos:"http" !== 0 && $periode.lien|strpos:"\\" !== 0}http://{$periode.lien}" target="_blank"><i class="icon-share"></i></a>
			
		</div>
	</td>
</tr>
<tr>
	<td >{#winPeriode_user2#} :</td>
	<td>
